...
export async function selectThrottlingOption() {
  ...
  const throttlingOptions = Object.entries(throttlingProfiles).map(([name]) => {
    return name;
  });
  console.log("Wähle eine Throttling-Option:");
  throttlingOptions.forEach((option, index) =>
    console.log(`${index}) ${option}`),
  );

  return new Promise((resolve) => {
    rl.question("Auswahl: ", (answer) => {
      const selectedOption = parseInt(answer, 10);
      if (selectedOption >= 0 && selectedOption < throttlingOptions.length) {
        rl.close();
        resolve([
          throttlingOptions[selectedOption],
          throttlingProfiles[throttlingOptions[selectedOption]],
        ]);
      } else {
...
export async function selectFramework(frameworks) {
  ...
  console.log("Wähle ein Framework oder alle:");
